<template>
    ...
    <div class="input-group" style="width: 200px;" v-if="esSuperusuario">
      <span class="input-group-text bg-white"><i class="bi bi-person"></i></span>
      <select class="form-select" v-model="filtroUsuario" @change="cargarRegistros">
...
const usuariosImportadores = ref([])
...
const cargarMesesDisponibles = async () => {
    try {
        const response = await api.get('/dimon/consultas-externas/meses-disponibles/')
...
const exportToExcel = async () => {
    try {
        ...
        const response = await api.get('/dimon/consultas-externas/exportar-todos/', {
            params: {
...
// Métodos para listado - DEJA SOLO ESTA VERSIÓN (elimina la otra)
const cargarRegistros = async () => {
  try {
    ...
    const response = await api.get('/dimon/consultas-externas/', { params })
...
const verificarSuperusuario = () => {
  try {
    const isSuperuser = localStorage.getItem('is_superuser') === 'true';
    esSuperusuario.value = isSuperuser;
    console.log('Estado de superusuario desde localStorage:', isSuperuser); // Para depuración
  } catch (error) {
...
const cargarUsuariosImportadores = async () => {
  if (!esSuperusuario.value) return;
  
  try {
    loading.value = true;
    const response = await api.get('/dimon/consultas-externas/usuarios-importadores/');
