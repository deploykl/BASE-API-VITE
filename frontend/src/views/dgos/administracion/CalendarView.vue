...
// Configuración de iconos para Leaflet (necesario para el marcador)
...
const initMap = () => {
  ...
  const lat = parseFloat(selectedComision.value.ipress_norte);
  const lng = parseFloat(selectedComision.value.ipress_este);
...
const calculateRoute = async () => {
  if (!originAddress.value.trim()) {
    alert('Por favor ingrese una dirección de partida');
    return;
  }

  try {
    // Geocodificar la dirección de partida
    const startLocation = await geocodeAddress(originAddress.value);
    ...
    startMarker = L.marker([startLocation.lat, startLocation.lng]).addTo(map)
      .bindPopup(`<b>Partida:</b> ${startLocation.address}`);

    // Obtener ruta de OSRM
    const response = await fetch(
      `https://router.project-osrm.org/route/v1/driving/${startLocation.lng},${startLocation.lat};${endLng},${endLat}?overview=full&geometries=geojson`
    );
